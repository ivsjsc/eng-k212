...
const FptLoginModal: FC<Props> = ({ open, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Notify caller so they can wire this up to Firebase or their auth flow.
      await Promise.resolve(onSubmit ? onSubmit({ displayName, email, password }) : undefined);
    } catch (err: any) {
      ...
      <div className="relative w-full max-w-md mx-4">
        <div className="bg-white/90 rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="p-6 pt-8">
            <h3 className="text-center text-orange-500 font-semibold text-lg mb-4">ĐĂNG KÝ</h3>

            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                <label className="sr-only">Tên hiển thị</label>
                <input
                  value={displayName}
              ...
              <div>
                <label className="sr-only">Gõ lại mật khẩu</label>
                <input
              ...
              {/* reCAPTCHA placeholder - swap with real reCAPTCHA if desired */}
              ...
              <div className="text-center text-sm text-gray-600 mt-2">
                Bạn đã có tài khoản?{' '}
                <button type="button" onClick={() => alert('Chuyển sang form Đăng nhập')} className="text-orange-500 font-medium">
                  Đăng nhập
                </button>
              </div>
            </form>
